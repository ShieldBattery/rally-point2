...
portable client, a shared per-leg transport layer, a validating relay + mesh,
...
| `transport`   | Per-leg reliable delivery over unreliable QUIC datagrams (ack + redundancy + sequence buffer). Shared by `client` and `relay` — one instance per leg.        |
| `client`      | Portable client endpoint, consumed by the ShieldBattery game DLL. Runs `transport` for its home-relay leg. Builds for `i686-pc-windows-msvc`.                |
