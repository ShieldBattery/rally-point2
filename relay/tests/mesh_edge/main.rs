...
struct Relay {
    ...
    sessions: Sessions,
    mesh: mesh::MeshState,
    mesh_rx: mpsc::Receiver<noq::Connection>,
}
...
async fn mesh_link_pair() -> (MeshLink, MeshLink, noq::Endpoint, noq::Endpoint) {
    let (chain, key, ca) = self_signed();
...
async fn next_mesh_presence(stream: &mut noq::RecvStream) -> MeshPresence {
