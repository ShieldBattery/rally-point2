...
impl DecisionMakers {
    ...
    pub fn note_slot_connected(&self, key: &SessionKey, slot: SlotId, resumed: bool) {
    ...
    /// union regardless, and — like [`note_slot_connected`](Self::note_slot_connected) — a session with no
    ...
    /// [`note_peer_slot_started`](Self::note_peer_slot_started), which folds a peer relay's shared report into
    /// the maker and stops there.
    pub fn note_slot_started(&self, key: &SessionKey, slot: SlotId) {
        if let Some(maker) = self.lock().get_mut(key) {
    ...
    /// from [`note_slot_started`](Self::note_slot_started): the home already reported the slot, and a
    /// second relay reporting it would attribute one client's load twice.
    pub fn note_peer_slot_started(&self, key: &SessionKey, slot: SlotId) {
        if let Some(maker) = self.lock().get_mut(key) {
    ...
    pub fn has_started(&self, key: &SessionKey, slot: SlotId) -> bool {
        self.lock()
            .get(key)
    ...
    pub fn started_home_slots(&self, key: &SessionKey) -> Vec<SlotId> {
        self.lock()
            .get(key)
    ...
    #[must_use]
    pub fn note_slot_present(&self, key: &SessionKey, slot: SlotId) -> bool {
        ...
        self.note_start_latched(key, fired);
        fired.is_some()
    }
    ...
    /// exists.
    #[must_use]
    pub fn reevaluate_start(&self, key: &SessionKey) -> bool {
        let fired = {
            let mut makers = self.lock();
            match makers.get_mut(key) {
                Some(maker) => maker
    ...
    /// through [`note_slot_present`](Self::note_slot_present) or [`reevaluate_start`](Self::reevaluate_start) — so it is the
    ...
    pub fn is_started(&self, key: &SessionKey) -> bool {
        self.lock().get(key).is_some_and(DecisionMaker::is_started)
    }
    ...
    /// [`is_started`](Self::is_started) so a caller sizing started-session state gets the gate
    /// and the shape in one registry lock; the count itself is
    /// [`DecisionMaker::session_slot_count`], including its `0`-means-unknown
    /// convention.
    pub fn started_session_slot_count(&self, key: &SessionKey) -> Option<usize> {
    ...
    pub fn mark_started(&self, key: &SessionKey) {
        if let Some(maker) = self.lock().get_mut(key) {
            maker.mark_started();
        }
    }
    ...
    pub fn initial_buffer_turns(&self, key: &SessionKey) -> Option<u32> {
        self.lock()
            .get(key)
    ...
    pub fn adopt_session_start(&self, key: &SessionKey, initial_buffer_turns: Option<u32>) {
        if let Some(maker) = self.lock().get_mut(key) {
