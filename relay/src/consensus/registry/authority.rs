...
impl DecisionMakers {
    ...
    #[must_use]
    pub fn sync_maker(&self, key: &SessionKey, sync: MakerSync<'_>) -> Vec<LeaveDirective> {
    ...
    /// `last_frame`.
    pub fn slot_frame(&self, key: &SessionKey, slot: SlotId) -> Option<GameFrameCount> {
        self.lock()
            .get(key)
    ...
    pub fn is_authority(&self, key: &SessionKey) -> bool {
        self.lock()
            .get(key)
    ...
    pub fn observe_frame(&self, key: &SessionKey, slot: SlotId, frame: GameFrameCount) {
    ...
    pub fn delivery_view(&self, key: &SessionKey) -> (Option<u64>, Option<u32>) {
        self.lock()
            .get(key)
